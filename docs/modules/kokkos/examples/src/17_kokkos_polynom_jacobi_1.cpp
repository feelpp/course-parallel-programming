...
double jacobi_polynomial(int n, double x, double alpha, double beta) {
  if (n == 0)
    return 1.0;
  if (n == 1)
    return 0.5 * (alpha + beta + (alpha - beta) * x);
  ...
  for (int k = 2; k <= n; ++k) {
    p_n = ((2 * k + alpha + beta - 1) * (p1 + (alpha - beta) * p0)) /
          (k + alpha + beta);
    p_n -= ((k + alpha - 1) * (k + beta - 1) * p0) /
           ((k - 1) * (k + alpha + beta));
    p0 = p1;
    p1 = p_n;
  }
...
struct JacobiKernel {
  ...
  JacobiKernel(int n_, double alpha_, double beta_,
               Kokkos::View<double *> x_vals, Kokkos::View<double *> res)
...
int main(int argc, char *argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    Kokkos::parallel_for(
        "InitX Values", num_points, KOKKOS_LAMBDA(const int i) {
          x_values(i) =
              -1.0 +
              2.0 * i / (num_points - 1); // Values ​​between -1 and 1
        });
    ...
    Kokkos::parallel_for("Jacob Eval", num_points,
                         JacobiKernel(n, alpha, beta, x_values, results));
    ...
    for (int i = 0; i < num_points; ++i) {
      std::cout << "P_" << n << "^(" << alpha << ", " << beta << ")("
                << h_results(i) << ") = " << h_results(i) << "\n";
    }
