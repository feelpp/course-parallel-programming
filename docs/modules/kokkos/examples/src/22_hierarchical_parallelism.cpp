...
Kokkos::Timer timer;
    ...
    // Hierarchical parallelism
      Kokkos::parallel_reduce("HierarchicalSum", 
      Kokkos::TeamPolicy<>(N / (TEAM_SIZE * VECTOR_SIZE), TEAM_SIZE, VECTOR_SIZE),
      KOKKOS_LAMBDA(const Kokkos::TeamPolicy<>::member_type& team_member, double& team_sum) {
        ...
        Kokkos::parallel_reduce(
          ...
          [&](const int vector_rank, double& vector_sum) {
            const int i = (league_rank * team_size + team_rank) * VECTOR_SIZE + vector_rank;
            if (i < N) {
              vector_sum += data(i);
            }
          },
