...
int main( int argc, char* argv[] )
{
    ...
    Kokkos::parallel_for("OuterLoop", team_policy(N, Kokkos::AUTO), KOKKOS_LAMBDA(const member_type& team_member) {
    ...
    double sum = 0.0;
    Kokkos::parallel_reduce("Sum", team_policy(N, Kokkos::AUTO), KOKKOS_LAMBDA(const member_type& team_member, double& lsum) {
      const int i = team_member.league_rank(); // Get the "outer" index
      double row_sum = 0.0;

      // Compute the row sum using nested parallelism
      Kokkos::parallel_reduce(Kokkos::TeamThreadRange(team_member, M), [=](const int j, double& thread_sum) {
